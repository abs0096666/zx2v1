...
def Ckuser(message):
  ...
  try:
    response = requests.get('https://almohajrina_walansar.ml/join.php?id={}'.format(userID)).json()
    if response["ok"]:
      return True
    elif response["ok"] == True:
      kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/almohajrina_walansar")] ])
